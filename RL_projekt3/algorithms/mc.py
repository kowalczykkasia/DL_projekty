...
def mc_prediction(env, policy: Callable[[int], int], nS: int, gamma: float = 0.99, episodes: int = 10_000, max_steps: int = 10_000):
    ...
    for _ in range(episodes):
        ...
        Gs = returns_from_trajectory(traj, gamma)  # Gs[t] ma być zwrotem od kroku t (czyli suma zdyskontowanych nagród od t do końca epizodu)

        visited = set()  # TODO: stwórz zbiór visited do first-visit (żeby aktualizować stan tylko raz na epizod)
        ...
        for (t, (s, a, r)) in enumerate(traj):
            ...
            visited.add(s)  # TODO: dodaj bieżący stan s do visited (oznaczamy, że już był)

            G = Gs[t]  # TODO: wybierz zwrot dla kroku t (to jest target do aktualizacji V[s])

            N[s] += 1  # TODO: zwiększ licznik wizyt stanu s (potrzebny do średniej inkrementalnej)

            # TODO: aktualizacja średniej inkrementalnej V[s] zgodnie ze wzorem: V <- V + (G - V)/N
            V[s] += (G - V[s]) / N[s]  # aktualizujemy estymatę wartości stanu s na podstawie zwrotu G
...
def mc_control_epsilon_greedy(env, nS: int, nA: int, gamma: float = 0.99, episodes: int = 50_000, epsilon: float = 0.1, seed: int = 0, max_steps: int = 10_000):
    ...
    def policy(s: int) -> int:
        return epsilon_greedy(Q[s], epsilon=epsilon, rng=rng)  # TODO: wybierz akcję ε-greedy na podstawie Q[s] (użyj helpera epsilon_greedy z utils)

    for _ in range(episodes):
        traj = rollout_episode(env, policy, max_steps=max_steps)

        # TODO: policz zwroty G_t dla każdego kroku t (użyj returns_from_trajectory z utils)
        Gs = returns_from_trajectory(traj, gamma)  # Gs[t] ma być zwrotem od kroku t (target do aktualizacji Q)

        visited = set()  # TODO: zbiór visited dla first-visit par (s,a) w tym epizodzie

        for (t, (s, a, r)) in enumerate(traj):
            key = (s, a)  # TODO: zdefiniuj klucz pary (s,a), np. (s, a)
            ...
            visited.add(key)  # TODO: dodaj key do visited (oznaczamy, że para (s,a) już była)

            G = Gs[t]  # TODO: wybierz zwrot dla kroku t: G = Gs[t] (to jest target dla Q[s,a])

            N[s, a] += 1  # TODO: zwiększ licznik wizyt pary (s,a) (potrzebny do średniej inkrementalnej)

            # TODO: aktualizacja średniej inkrementalnej Q[s,a] zgodnie ze wzorem: Q <- Q + (G - Q)/N
            Q[s, a] += (G - Q[s, a]) / N[s, a]  # aktualizujemy wartość akcji a w stanie s na podstawie zwrotu G
